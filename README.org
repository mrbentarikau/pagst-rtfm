...
{{if $msgid := (dbGet 0 "faq").Value }}
    {{ if eq .Reaction.MessageID (toInt $msgid) }}
        {{ if eq .Reaction.Emoji.Name "1??" }}
            {{sendDM "One!"}}
        {{ else if eq .Reaction.Emoji.Name "2??" }}
            {{sendDM "Two!"}}
        {{ else if eq .Reaction.Emoji.Name "3??" }}
            {{sendDM "Three!"}}
        {{ else if eq .Reaction.Emoji.Name "4??" }}
            {{sendDM "Four!"}}
        {{ else if eq .Reaction.Emoji.Name "5??" }}
            {{sendDM "Five!"}}
        {{end}}
    {{end}}
{{end}}?
